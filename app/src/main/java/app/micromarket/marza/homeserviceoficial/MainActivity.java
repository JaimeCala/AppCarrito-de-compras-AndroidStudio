...
public class MainActivity extends AppCompatActivity {
    ...
    private TextView registrarse, showResetPassword ;
    private EditText usuario, forgot_email;
    private TextInputLayout contrasena;
    private Button login, btnForgotPass;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //recuperacion de contraseña
         showResetPassword = findViewById(R.id.txtResetPassword);
         showResetPassword.setOnClickListener(resetpassword->{
             mostrarResetPasswordDialog();
         });
    ...
    private void mostrarResetPasswordDialog() {
        View forgot_password_layout = LayoutInflater.from(this).inflate(R.layout.forgot_password,null);
        forgot_email = forgot_password_layout.findViewById(R.id.forgot_email);
        btnForgotPass = forgot_password_layout.findViewById(R.id.forgot_password_button);
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("RESTABLECER CONTRASEÑA");
        builder.setView(forgot_password_layout);
        //builder.setCancelable(false);
        final AlertDialog dialog = builder.create();
        btnForgotPass.setOnClickListener(forgotpass ->{
            dialog.dismiss();

            if(TextUtils.isEmpty(forgot_email.getText().toString().trim()) || !PatternsCompat.EMAIL_ADDRESS.matcher(forgot_email.getText().toString().trim()).matches())
            {
                Toast.makeText(MainActivity.this, "Introduzca un correo electrónico válido", Toast.LENGTH_SHORT).show();
                return;

            }

            Toast.makeText(MainActivity.this, " no valida correo", Toast.LENGTH_SHORT).show();

            String correoUser = forgot_email.getText().toString().trim();
            //enviar email
            compositeDisposable.add(
                    mService.sendEmailResetPassword(correoUser)
                            .observeOn(AndroidSchedulers.mainThread())
                            .subscribeOn(Schedulers.io())
                            .subscribe(resSendEmail ->{

                                //esperarDialog.dismiss();
                                Toast.makeText(MainActivity.this, " dentro de composite", Toast.LENGTH_SHORT).show();
                                if(TextUtils.isEmpty(resSendEmail))
                                {
                                    Toast.makeText(MainActivity.this, " "+resSendEmail, Toast.LENGTH_SHORT).show();
                            ...
                            },throwable -> {Toast.makeText(MainActivity.this, "[Correo enviado]"+throwable.getMessage(), Toast.LENGTH_SHORT).show();}) );
...
//---------------------------
                ...
                if(TextUtils.isEmpty(correo.getText().toString() ) || !PatternsCompat.EMAIL_ADDRESS.matcher(forgot_email.getText().toString().trim()).matches())
                {
                    Toast.makeText(MainActivity.this, "Introduzca su correo electrónico válido", Toast.LENGTH_SHORT).show();
                    return;
                }
                                        ...
                                        compositeDisposable.add(
                                                mService.createLogin(users.getIdusuario(), correoUser,passworUser,currentDate,currentTime)
                                                        ...
                                                        .subscribe(Login ->{
                                                            ...
                                                            esperarDialog.dismiss();
                                                            app.micromarket.marza.homeserviceoficial.Model.Login login = Login;
